!   Compute the first four sample moments for grid-point daily temperature data
!   read from input netcdf files
!   ---------------------------------------------------------------------------
!   September 2022 
...
!   auxlib.a (only: Calc_time.f90, Fix_parameters.f90, Calc_days.f90, Calc_ctmoms.f90)
!
!   Input: 
!   - list of time-ordered nc files to be processed, with the same variables' and grid structure; 
!   files include daily data of one or more years and cover the period for calculation of moments;
...
!
!   Output:
!   - first four sample central moments as a function of time for every grid point, printed as single  
!   precision real on ascii files named 'fout-smp_lon_lat'; 'fout' (max a20) is defined as input, 'smp' 
!   (a3, input) is a label specifying data selection (year/season), 'lon'='xxxyy' (xxx.yy in 0,360) 
!   and 'lat'='+/-xxyy' (+/- xx.yy in -90,+90); missing values are set NaN;
!   - list of output files with grid-point coordinates ('ls_fout-smp') ; 
!   - list of grid points with undefined moments ('fout-smp.log'); 
!
...
!     Output & settings
      ...
      real(4) :: const
      logical :: cdev
      integer(4) :: y1sp, y2sp,isn, nrchar, enchar, ochar 
      integer(4) :: y1,yN, nnr, xlen, jjmin, ngp, inr
      integer(4) :: nbk, jbk, ix1, ix2, ixx, nxx
      integer(4), parameter :: st0=1, st1=0, st2=0  !loop step, L and R window (years)
      integer(4) :: id, j1,j2, jj, kw, ii1, ii2, iw
      real, allocatable :: xa(:,:), xh(:), norm(:)
      character(200) :: nrpath, nrname, opath
      character(20) :: fout                                                  
      character(3) :: smp
      character(5) :: chlon, chlat                                          
      integer(4) :: fchar, flon, flat
      character(50) :: fileout                                              
      real :: mu, sig2, sig, skew, kurt
      real(4) :: t0c, tfc
      real(8) :: t0w, tfw
      integer(4) :: yy0, yyf
      ...
      print*,' '
      print*,'Start program: CMoments_nc'
      print*,''
      print*,'--> SET INPUT '   
      print*,' '
...
!     Enter boundary years for calculation of moments
      read*, y1sp, y2sp
...
!     Enter path (max a200) for output files      
      read(*,'(a)') opath
      do i = 1,200
...
!     Enter base name (max a20) of output files 
      ...
      write(*,'(1x,a)')'Input stored in file inp_mm'
      print*,' '
...
!     Allocate arrays 
      ...
      if(y1sp.lt.y0(1).or.y2sp.gt.yF(nset))then
         print*,'Error: period for  moments not included in input file(s)'
         print*,'Stop. Check input list!'
         stop
      endif
      ...
      if(gap.gt.0)then
         do i =1,gap
            if(y2sp.lt.yG1(i).or.y1sp.gt.yG2(i))then
               ...
               write(*,'(1x,a,2I5)')'Error: gap in the period of moments: ',yG1(i),yG2(i)
               print*,'Stop. Check input list!'
               stop
            endif
         enddo
...
!     Store coordinates into program arrays       
      ...
      write(*,'(3x,a,i2,2x,a)')'Get var #',vidlon,vrlon   !longitude   
      ...
      write(*,'(3x,a,i2,2x,a)')'Get var #',vidlat,vrlat   !latitude
      ...
      open(20, file = opath(1:ochar)//'ls_'//fout(1:fchar)//'-'//smp, status = 'unknown')
      open(22, file = opath(1:ochar)//fout(1:fchar)//'-'//smp//'.log', status ='unknown')  
...
!     Cycle on grid points in each block
         do ilat = ix1, ix2
            ...
            do ilon = 1,Nlon 
               ...
               ngp = ngp + 1
               fileout = fout(1:fchar)//'-'//smp//'_'//chlon//'_'//chlat
               write(20,*) fileout,lons_in(ilon), lats_in(ilat)
            
!     Read grid-point normals from files
               if(cdev)then
                  open(24, file = nrpath(1:nrchar)//nrname(1:enchar)//'_'//chlon//'_'//chlat, status = 'old')
                  do inr = 1,nnr
                     read(24,*,iostat=ios) id, norm(inr)
                     if(ios.lt.0) exit 
                  enddo
                  close(24)
               endif

!     Store grid-point data into xa(id,k). If ileap=2 and k nonleap, xa(366,k) is nan
               do iset = 1,nset
                  itm = 0            
...
!     Compute first four moments at each grid point from annual/seasonal samples, provided that        
!     a minimum # data is reached. If not, subrout 'moments' returns NaN
               open(26, file = opath(1:ochar)//fileout, position = 'append')
...
!     DJF sample (Dec)
                     if(kw.eq.y1) cycle
                     do iw = iws(4,kw-1),nid(kw-1) 
                        if(isnan(xa(iw,kw-1))) cycle
                        jj = jj+1
                        xh(jj) = xa(iw,kw-1)
                     enddo         !iw
                  ...
                  if(jj.lt.jjmin)then
                     jj = 0           
                     write(22,*) lons_in(ilon), lats_in(ilat), rnan
                  endif

                  call moments(jj,xh,mu,sig2,sig,skew,kurt) 
                  write(26,*) k, mu, sig, skew, kurt
               enddo  !k
               close(26)
      ...
      write(*,'(5x,a,1x,a)')'Output printed on: ',opath(1:ochar)//fout(1:fchar)//'-'//smp//'_*'
      print*,' '
